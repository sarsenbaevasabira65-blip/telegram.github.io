...
# config.txt файлынан оқу (бар болса)
...
def save_config(config: dict):
    with open(CONFIG_FILE, "w", encoding="utf-8") as f:
        f.write("# Бот баптаулары — осы файлды өзгертіңіз\n\n")
        for key, val in config.items():
            f.write(f"{key} = {val}\n")

def setup():
    """Бірінші іске қосылғанда деректерді сұрайды"""
    config = load_config()

    if not config.get("BOT_TOKEN") or config["BOT_TOKEN"] == "СІЗДІҢ_BOT_TOKEN":
        print("\n" + "="*50)
        print("🤖 «Сенің идеяң — біздің өнер» — Бот баптауы")
        print("="*50)
        print("\n@BotFather-дан алған токенді енгізіңіз:")
        config["BOT_TOKEN"] = input("BOT_TOKEN: ").strip()

    if not config.get("ADMIN_ID") or config["ADMIN_ID"] == "123456789":
        print("\nСіздің Telegram ID-ыңызды енгізіңіз (@userinfobot арқылы алыңыз):")
        config["ADMIN_ID"] = input("ADMIN_ID: ").strip()

    if not config.get("KASPI_NUMBER"):
        print("\nKaspi нөміріңізді енгізіңіз (мыс: +77001234567):")
        config["KASPI_NUMBER"] = input("KASPI_NUMBER: ").strip()

    if not config.get("KASPI_NAME"):
        print("\nKaspi-дегі атыңызды енгізіңіз:")
        config["KASPI_NAME"] = input("KASPI_NAME: ").strip()

    if not config.get("CARD_NUMBER"):
        print("\nБанк карта нөміріңізді енгізіңіз (бос қалдыру үшін Enter басыңыз):")
        val = input("CARD_NUMBER: ").strip()
        config["CARD_NUMBER"] = val if val else "—"

    if not config.get("CARD_BANK"):
        print("\nБанк атауын енгізіңіз (мыс: Halyk):")
        val = input("CARD_BANK: ").strip()
        config["CARD_BANK"] = val if val else "—"

    save_config(config)
    print("\n✅ Баптаулар сақталды! (config.txt)\n")
    return config
...
KASPI_NUMBER  = _cfg.get("KASPI_NUMBER", "+7 700 000 00 00")
KASPI_NAME    = _cfg.get("KASPI_NAME", "—")
CARD_NUMBER   = _cfg.get("CARD_NUMBER", "—")
CARD_BANK     = _cfg.get("CARD_BANK", "—")
...
def payment_keyboard(lang, svc_key):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📱 Kaspi QR / Нөмір",          callback_data=f"pay_kaspi_{svc_key}_{lang}")],
